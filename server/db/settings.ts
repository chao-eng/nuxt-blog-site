...
export function initSettingsTables(): void {
  ...
  const umamiTableInfo = db.prepare('PRAGMA table_info(\'umami_config\')').all() as { name: string }[]
  ...
  // 检查是否已有 path 列（用于迁移）
  const s3TableInfo = db.prepare('PRAGMA table_info(\'s3_config\')').all() as { name: string }[]
